...
.PHONY: build/pager-cli-client

build/pager-cli-client:
	mkdir -p ./build
	go build -o ./build/pager-cli-client main.go

build: build/pager-cli-client

install:
	mkdir -p ~/.local/bin
	cp ./build/pager-cli-client ~/.local/bin/pager-cli-client
